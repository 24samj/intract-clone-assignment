...
function Quest({ quest, title, subtitle }) {
    ...
    return (
        <details
            ...
            <summary className="flex h-full w-full bg-[#151515] p-4 hover:bg-[#313131] transition-all duration-300 ease-in-out">
                <div className="relative w-[35%] border-2 rounded-2xl border-gray-500 border-opacity-20 flex justify-center">
                    <Image
                        ...
                        priority
                    />
                    <h4 className="absolute bottom-1 w-min text-nowrap border border-gray-500 border-opacity-20 text-center text-sm text-white backdrop-blur-lg bg-gray-500 bg-opacity-40 py-2 px-6 rounded-full">
                        {quest.length} Tasks
                    </h4>
                </div>
                <div className="w-[65%] text-left p-4">
                    <h3 className="text-3xl text-ellipsis">{title}</h3>
                    <p className="mt-2 border-b-2 border-dotted border-gray-500 border-opacity-20 pb-4 text-sm text-gray-400">
                        {subtitle}
                    </p>
